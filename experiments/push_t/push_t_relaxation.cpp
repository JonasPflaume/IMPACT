...
bool parseArgs(int argc, char* argv[], casadi::DM& x0, casadi::DM& x_goal,
    ...
    if (argc >= 8) {
        ...
        oss << "results/push_t/relaxation/trajectory_" << timestamp << ".txt";
        output_file = oss.str();
    }
...
void saveSolutionToFile(const aula::ScholtesRelaxationSolution& solution, const casadi::DM& x0,
    ...
    file << "# PushT Scholtes Relaxation Trajectory\n";
...
int main(int argc, char* argv[]) {
    std::cout << "=== PushT MPCC with Scholtes Relaxation Solver ===" << std::endl;
    ...
    config.stage_state_cost_weight = 0.0;  // Stage state tracking
    ...
    // IPOPT options
    config.ipopt_print_level = 0;  // Quiet IPOPT
    ...
    std::cout << "  Stage state cost weight: " << config.stage_state_cost_weight << std::endl;
    ...
    aula::MPCCScholtesRelaxationSolver solver(push_t_problem);
