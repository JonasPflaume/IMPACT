...
Usage:
    python python/examples/push_circle/main.py --visualize
    python python/examples/push_circle/main.py --render-only [trajectory.txt]
...
from ..common.trajectory_io import resolve_output_dir
...
def visualize_trajectory(R, goal, X, U, fps=25, stride=1, out=None, minimal=False):
    """Render a trajectory supplied directly as time-major state/control arrays."""
    goal = np.asarray(goal, dtype=float).ravel()
    X = np.asarray(X, dtype=float)
    U = np.asarray(U, dtype=float)
    if X.ndim != 2 or X.shape[1] != 4:
        raise ValueError(f"X must have shape (T+1, 4), got {X.shape}")
    if U.ndim != 2 or U.shape[1] != 4 or U.shape[0] != X.shape[0] - 1:
        raise ValueError(f"U must have shape (T, 4), got {U.shape} for X {X.shape}")
    ...
    if out is None:
        out = resolve_output_dir("push_circle") / "push_circle.gif"
...
def visualize(path, fps=25, stride=1, out=None, minimal=False):
    """Read a trajectory file and render it."""
    R, goal, X, U = parse_trajectory(path)
    if out is None:
        out = os.path.splitext(path)[0] + ".gif"
    return visualize_trajectory(R, goal, X, U, fps=fps, stride=stride,
                                out=out, minimal=minimal)


def render(trajectory, *, out=None, out_dir=None, minimal: bool = False, fps: int = 25,
           stride: int = 1, **_) -> List[Path]:
    """The entry point ``main.py`` calls: draw a saved trajectory file.

    The goal and the disk radius are read from the file rather than passed in, so
    the picture always describes the trajectory it is drawn from.
    """
    path = Path(out) if out else resolve_output_dir("push_circle", out_dir) / "push_circle.gif"
    visualize(str(trajectory), fps=fps, stride=stride, out=str(path), minimal=minimal)
    # visualize_trajectory writes the static overlay beside the GIF.
    return [path, path.with_name(path.stem + "_overlay.png")]
