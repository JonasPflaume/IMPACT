...
def render(trajectory, *, goal=None, out_dir=None, minimal: bool = False,
           frame_interval: int = 10, **kwargs) -> List[Path]:
    """The entry point ``main.py`` calls: draw a saved trajectory file.

    The gap length and the time step come from the model rather than from
    defaults repeated here, so the drawing cannot end up describing a different
    track than the one that was solved.
    """
    from .task import CartTransporter

    cart = CartTransporter()
    goal = recorded_goal(trajectory) if goal is None else goal
    output_dir = resolve_output_dir("cart_transporter", out_dir)
    visualize_solution_from_file(str(trajectory), l=cart.l, dt=cart.dt,
                                 x_goal=None if goal is None else list(goal),
                                 minimal_overlay=minimal, frame_interval=frame_interval,
