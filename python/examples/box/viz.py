#!/usr/bin/env python3
"""
Box-pushing trajectory visualizer.
...
from ..common import show_if_interactive
from ..common.trajectory_io import recorded_goal, resolve_output_dir
...
def visualize_solution_from_file(trajectory_file, a=0.3, b=0.4, x_goal=None, dt=0.1,
    ...
    # Parse control trajectory (cx, cy, ld1y, ld2x, ld3y, ld4x)
    control_lines = []
    ...
    u = np.array(u_data)  # Shape: (timesteps, 6) - each row is [cx, cy, ld1y, ld2x, ld3y, ld4x]
    
    print(f"Loaded trajectory: {x.shape[0]} timesteps, state dim={x.shape[1]}, control dim={u.shape[1]}")
    
    # Local rectangle corners (centered)
    local_corners = np.array([[-a, -b],
                              [ a, -b],
                              [ a,  b],
                              [-a,  b]])
    ...
    # Plot the path behind the moving box.
    ...
    poly = mpatches.Polygon(corners0, closed=True, facecolor='#4da6ff', 
                           edgecolor='#0066cc', linewidth=2.5, alpha=0.7, label='Box')
    ...
    ctrl_scatter = ax.scatter(ctrl_world[0,0], ctrl_world[0,1], c='#cc00cc', s=120, zorder=5, 
                              label='Contact Point', marker='*', edgecolors='#660066', linewidths=1.5)

    # Force arrow scale factor (smaller = shorter arrows)
    force_scale = 0.3

    # Define four presumed force contact locations in body frame
    force_bodies = np.array([[-a, 0.0],
                              [0.0, -b],
                              [ a, 0.0],
                              [0.0,  b]])
    
    # Initial force values
    fvals0 = np.array([u[0,2], u[0,3], u[0,4], u[0,5]]) if u.shape[0] > 0 else np.zeros(4)
    fvecs_body0 = np.array([[0.0, fvals0[0]],
                             [fvals0[1], 0.0],
                             [0.0, fvals0[2]],
                             [fvals0[3], 0.0]]) * force_scale
    origins0 = np.full_like(force_bodies, [ctrl_world[0,0], ctrl_world[0,1]])
    fvecs_world0 = (fvecs_body0 @ R0.T)

    # Force arrows.
    fx = fvecs_world0[:,0]
    fy = fvecs_world0[:,1]
    fq = ax.quiver(origins0[:,0], origins0[:,1], fx, fy, angles='xy', scale_units='xy', scale=1,
                   color='#009900', width=0.007, label='Contact Forces', alpha=0.8,
                   headwidth=4, headlength=5, headaxislength=4)
    ...
    if x_goal is not None:
        ...
        goal_circle = Circle((cx_goal, cy_goal), radius=0.05, color='#ff9900', alpha=0.7, zorder=3)
        ax.add_patch(goal_circle)

    # Axis limits include the full box footprint.
    x_min = x[:,0].min() - a - 0.2
    x_max = x[:,0].max() + a + 0.2
    y_min = x[:,1].min() - b - 0.2
    y_max = x[:,1].max() + b + 0.2
    ...
    if x_goal is not None:
        x_min = min(x_min, x_goal[0] - a - 0.2)
        x_max = max(x_max, x_goal[0] + a + 0.2)
        y_min = min(y_min, x_goal[1] - b - 0.2)
        y_max = max(y_max, x_goal[1] + b + 0.2)
    ...
    def update(i):
        ...
        # Forces
        if i < u.shape[0]:
            fvals = np.array([u[i,2], u[i,3], u[i,4], u[i,5]])
        else:
            fvals = np.array([u[-1,2], u[-1,3], u[-1,4], u[-1,5]])
        fvecs_body = np.array([[0.0, fvals[0]],
                                [fvals[1], 0.0],
                                [0.0, fvals[2]],
                                [fvals[3], 0.0]]) * force_scale
        origins = np.full_like(force_bodies, [ctrl_world[0,0], ctrl_world[0,1]])
        fvecs_world = (fvecs_body @ R.T)
        fq.set_offsets(origins)
        fq.set_UVC(fvecs_world[:,0], fvecs_world[:,1])
        ...
        if x_goal is not None:
            ...
            angle_diff = abs(th_i - x_goal[2]) * 180 / np.pi
        else:
            dist_goal = 0.0
            angle_diff = 0.0
        ...
        if info_texts is not None:
            ...
            info_texts['pos'].set_text(f'Pos: ({cx_i:+.3f}, {cy_i:+.3f})')
            info_texts['angle'].set_text(f'θ: {th_i*180/np.pi:+7.2f}°')
            info_texts['vel'].set_text(f'Vel: {vel:.3f} m/s')
            info_texts['dist'].set_text(f'Dist to Goal: {dist_goal:.3f}m')
            
            info_texts['contact'].set_text(f'Contact: ({ctrl_pt[0]:+.3f}, {ctrl_pt[1]:+.3f})')
            force_total = np.linalg.norm(fvecs_world.sum(axis=0))
            info_texts['forces'].set_text(f'F_ld: [{fvals[0]:+.2f}, {fvals[1]:+.2f},\n      {fvals[2]:+.2f}, {fvals[3]:+.2f}]')
            info_texts['force_mag'].set_text(f'|F_total|: {force_total:.3f}N')
    ...
    anim = animation.FuncAnimation(fig, update, frames=len(x), interval=max(50, int(dt*1000)),
    ...
    if not minimal_overlay:
        ...
        title = ax.set_title("MPCC Box Pushing Controller", 
    ...
    # Draw all box states with fading alpha
    n_frames = len(x)
    force_scale_static = 0.1  # Smaller scale for static overlay
    for i in range(n_frames):
        ...
        if i < u.shape[0]:
            ...
            # Forces: [ld1y, ld2x, ld3y, ld4x] -> net force in body frame
            fvals = np.array([ui[2], ui[3], ui[4], ui[5]])
            # Net force in body frame (simplified: sum of components)
            fx_body = fvals[1] + fvals[3]  # x direction forces
            fy_body = fvals[0] + fvals[2]  # y direction forces
            force_body = np.array([fx_body, fy_body]) * force_scale_static
            force_world = R @ force_body
            
            force_mag = np.sqrt(fx_body**2 + fy_body**2)
            if force_mag > 0.01:  # Skip tiny force arrows.
    ...
    if not minimal_overlay:
        ...
        ax_static.set_title(f"Box Pushing Trajectory ({n_frames} timesteps)", fontsize=14, fontweight='bold')
    ...
    try:
        ...
        writer = Writer(fps=15, metadata=dict(artist='BoxPushing'), bitrate=1800)
...
def render(trajectory, *, goal=None, out_dir=None, minimal: bool = False,
           **kwargs) -> List[Path]:
    """The entry point ``main.py`` calls: draw a saved trajectory file.

    The box geometry comes from the model rather than from defaults repeated
    here, so a change to :class:`~examples.box.task.BoxPushing` cannot leave the
    drawing describing a differently-shaped box than the one that was solved.
    """
    from .task import BoxPushing

    box = BoxPushing()
    goal = recorded_goal(trajectory) if goal is None else goal
    output_dir = resolve_output_dir("box", out_dir)
    visualize_solution_from_file(str(trajectory), a=box.a, b=box.b, dt=box.dt,
                                 x_goal=None if goal is None else list(goal),
                                 minimal_overlay=minimal, output_dir=output_dir,
                                 **kwargs)
    return [output_dir]
