...
def render(trajectory, *, goal=None, out_dir=None, minimal: bool = False,
    ...
    The T-shape and the limit-surface parameter come from the model rather than
    from defaults repeated here, so the drawing cannot end up describing a
    different T than the one that was solved.
    """
    from .task import PushT

    block = PushT()
    goal = recorded_goal(trajectory) if goal is None else goal
    output_dir = resolve_output_dir("push_t", out_dir)
    visualize_solution_from_file(str(trajectory), l=block.l, dc=block.dc, dt=block.dt,
